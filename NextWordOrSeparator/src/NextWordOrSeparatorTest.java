...
public final class NextWordOrSeparatorTest {
    ...
    private static String nextWordOrSeparator(String text, int position,
        ...
        ///one loop try:if they have different types then it breaks the loop
        int j = position;
        char start = text.charAt(j);
        while (j < text.length()) {
            char ch = text.charAt(j);
            if ((!separators.contains(ch) && !separators.contains(start))
                    || (!separators.contains(ch)
                            && !separators.contains(start))) {
                j++;
            } else {
                break;
            }

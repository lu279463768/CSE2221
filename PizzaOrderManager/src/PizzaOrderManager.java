...
public final class PizzaOrderManager {
        ...
        while (!reader.atEOS()) {
            ...
            if (!menu.contains(",")) {
                Reporter.fatalErrorToConsole(
                        "Violation of: each line in the file"
                                + " has word and price separated by ','");
            }
            str = menu.split(",");

            priceMap.add(str[0], Integer.parseInt(str[1]));
    ...
    private static int getOneOrder(SimpleReader input,
        ...
        /*
         * Check rest of precondition:
         */

        String firstLine = input.nextLine();
        if (!sizePriceMap.hasKey(firstLine)) {
            Reporter.fatalErrorToConsole(
                    "Violation of: input.content begins with a pizza order"
                            + " consisting of a size defined in sizePriceMap. The attempted input was "
                            + firstLine);
        }
        String secondLine = input.nextLine();
        if (!secondLine.isEmpty() && !toppingPriceMap.hasKey(secondLine)) {
            Reporter.fatalErrorToConsole(
                    "Violation of: input.content first line followed"
                            + " by zero or more toppings defined in toppingPriceMap");
        }

        int totalPrice = 0;
        String size = firstLine;
        totalPrice += sizePriceMap.value(size);
        /*
         * zero or more toppings' whole price:
         */
        String topping = secondLine;
        while (!topping.isEmpty() && !input.atEOS()) {
    ...
    private static void putPrice(SimpleWriter output, int price) {
        ...
        int cents = price;
        if (price > HUNDRED) {
            dollars = price / HUNDRED;
            cents = cents - dollars * HUNDRED;
        }
        output.println(
                "The price is " + dollars + " dollars, " + cents + " cents.");
